...
@section('content')
  ...
  @php $statuses = ['available' => 'success', 'in_service' => 'primary', 'out_of_service' => 'neutral']; @endphp
      ...
      <tbody>
        @foreach ($vehicles as $v)
          <tr>
            ...
            <td>{{ $v->capacity }}</td>
            <td><x-badge :variant="$statuses[$v->status] ?? 'neutral'">{{ ucfirst(str_replace('_', ' ', $v->status)) }}</x-badge></td>
            <td>{{ $v->notes ?? '—' }}</td>
