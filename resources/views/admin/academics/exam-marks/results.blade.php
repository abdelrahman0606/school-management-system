  ...
  <div class="d-flex justify-content-between align-items-start mb-3 flex-wrap gap-2">
    <div>
      ...
      <h1 class="h4 mb-0">Tabulation — {{ $exam->title }}
        @if ($locked)<x-badge variant="danger" class="align-middle"><i class="bi bi-lock-fill"></i> {{ __('Locked') }}</x-badge>@endif
      </h1>
        ...
        <tbody>
          @foreach ($rows as $r)
            <tr>
              ...
              <td>
                @if ($r->is_pass)<x-badge variant="success">{{ __('Pass') }}</x-badge>@else<x-badge variant="danger">{{ __('Fail') }}</x-badge>@endif
              </td>
            </tr>
          @endforeach
        </tbody>
